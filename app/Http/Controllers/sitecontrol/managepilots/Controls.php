...
class Controls extends MY_controller
{
	function __construct()
	{
		...
		$this->data['_controller']								= "managepilots";
		...
		$this->input_fields	 									= ['hub_id', 'rank_id', 'pilot_role_id', 'notifications', 'first_name', 'last_name', 'email', "bio", "call_sign", "number_flights|default_zero", "vatsim_id|default_zero", "user_image", 'id', 'option']; #'images_types|images_types'
		$this->fillable_inputs 									= ['hub_id', 'rank_id', 'pilot_role_id', 'notifications', 'first_name', 'last_name', 'email', "bio", "call_sign", "number_flights", "vatsim_id", "user_image", 'id', 'option']; #, 'images_types'
	...
	public function view(Request $request, Builder $htmlBuilder, string $view_filter = 'view')
	{
		...
		$tmp_query          = Pilots::query();
		if ( Auth::guard(RoleManagement::get_current_user_logged_in_GUARD())->check() )
		{
			$tmp_query->where("id", "!=", Auth::guard(RoleManagement::get_current_user_logged_in_GUARD())->user()->id);
		}

		$tmp_query->where("airline_id", get_airline_ID() );
		$tmp_query->whereIn("pilot_role_id", RoleManagement::get_pilot_ROLE_IDS_with_logged_in_ROLE_ID());
